...
# Iteration Settings
START_ITERATION_TIME = "2021-05-30T06:00:00" #This time is in UTC. Offset properly from local time zone
...
class FreeOsContract(object):
    ...
    def move_to_next_iteration_live(self, target_iter = None,
        ...
        if target_iter:
            ...
            self.proposed_iteration = target_iter
        else:
            self.get_current_iteration()
    ...
    def update_iteration(self, iter_num, start_time, end_time, claim, holding):
        ...
        self.show(f"Adding Pause of {ADD_PAUSE_SEC} seconds after updating iteration #{iter_num}")
        time.sleep(int(ADD_PAUSE_SEC))
        return s,o,e
    ...
    def get_defined_exchange_rate_for_next_iter(self, iter_num):
        ...
        try:
            ...
            self.show(f"Iteration {iter_num} not in the processed list [length={len(exchange_rates)}]")
        finally:
            self.show(f"{FILTER}{caller()}(): Iteration #{iter_num}: extracted exchange rate = {exchange_value} [length={len(exchange_rates)}]")
    ...
    def get_current_iteration(self):
        ...
        self.show(f"Actual current iteration: {current_iteration_num_dividend} [Dividend version: {out}]")
        if current_iteration_num_dividend == 0:
            self.show(f"No iteration is currently active. Changing Iter {current_iteration_num_dividend} to {self.proposed_iteration}")
            current_iteration_num_dividend = self.proposed_iteration
        self.show(f"All Iterations: {self.all_iterations}")
    ...
    def fetch_liquid_vest_unvest_allocation_rate(self):
        ...
        self.show(f"Stats:Unvest Percent={self.unvest_percent}, Current Rate={self.current_freeos_rate}")
...
def user_claim_simulation():
    ...
    current_rates=[500,0.0167, 0.011, 0.0018, 0.0167,
                   0.21, 9999999999999999.22, 0.00202, 1.019, 10.5, 1110.01, 977.01, 50.003,
                   0.001, 0.0000000001, 0.0009, 0.0081,
                   0.00821, 0.00167, 0.016666666, 20.01,
                   0.0013, 0.00032, 0.0029, 0.0081, 0.0001, 0.000810, 0.0009, 0.00851,0.0091, 0.0009,
                   0.0013, 0.00032, 0.00129, 0.00181, 0.00021, 0.000810, 0.0009, 0.00851,0.0091, 0.0004841,
                   0.0167,0.0167,0.0167,0.202, 1.019, 10.5, 1110.01, 977.01, 0.001003]

    current_rates.extend(current_rates)
    planned=[(i,["G","B"][i < float(TARGET_RATE)]) for i in current_rates]
    print(planned)
    print(f"Execution planned for good and bad rates: {len(current_rates)}, from {USERS_FILE}")
    ...
    freeos.unlock(CONFIG_SC)
    freeos.unlock(AIRCLAIM_SC)
    freeos.proposed_iteration = START_ITER_NUM
    freeos.get_current_iteration()
    ...
    for i in range( len(current_rates)):
        ...
        for auser in participating_users:
            auser.unlock_wallet()
            freeos.show(f"\n{FILTER}[Iter #{iter_num}] Processing for user: {auser.account}")
            vested=auser.get_options_vested()
            liquid=auser.get_options_account()
            freeos.show(f"{FILTER}[Iter #{iter_num}] {auser.account}: Before claim: vested={vested}, liquid={liquid},CurRate={current_rates[i]},Good Time={good_time}")
            s,o,e=auser.claim()
            extracted = freeos.locate_message_in_output(f"{auser.account} <= {AIRCLAIM_SC}::allocate ",o,e)
            freeos.show( f"{FILTER}[Iter #{iter_num}] {auser.account}: {extracted}:Claimed:{s},{o},{e}")
            s,o,e=auser.claim()
            freeos.show(f"{FILTER}{auser.account}: ReClaimed:{s},{o},{e}")
            vested=auser.get_options_vested()
            liquid=auser.get_options_account()
            freeos.show(f"{FILTER}{auser.account}: After claim: vested={vested}, liquid={liquid},CurRate={current_rates[i]},Good Time={good_time}")
            s,o,e=auser.unvest()
            extracted = freeos.locate_message_in_output(f"{auser.account} <= {AIRCLAIM_SC}::allocate ",o,e)
            if not extracted:
                extracted = freeos.locate_message_in_output('cannot be unvested in this claim period',o,e)
            freeos.show(f"{FILTER}{auser.account}: Good Time={good_time} {extracted},Unvest:{s},{o},{e}")
            s,o,e=auser.unvest()
            freeos.show( f"{FILTER}{auser.account}: Second Unvest:{s},{o},{e}")
            vested=auser.get_options_vested()
            liquid=auser.get_options_account()
            freeos.show(f"{FILTER}[Iter #{iter_num}]{auser.account}: After unvest(), vested={vested}, liquid={liquid},CurRate={current_rates[i]},Good Time={good_time}")
